...
class ETLController(BaseController):
    ...
    @ex(
        ...
        arguments=[
            (["--table"], {"help": "Table to insert into", "required": True}),
            (["--file"], {"help": "File where to find the data", "required": True}),
            (
                ["--format"],
                {
                    "help": "JSON/CSV, or wathever is supported by peewee/Dataset/Thaw",
                    "required": True,
                },
            ),
        ],
